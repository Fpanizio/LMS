...
export class Password {
    ...
    async hash(password: string) {
        ...
        const dk = await scryptAsync(password_hmac, salt, this.DK_LENGTH, this.SCRIPT_OPTIONS);
    ...
    async verify(password: string, password_hash: string) {
        ...
        const password_hmac = createHmac('sha256', this.PEPPER).update(password_normalized).digest();
        const dk = await scryptAsync(password_hmac, stored_salt, this.DK_LENGTH, stored_options);
